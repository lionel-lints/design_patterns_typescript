...
describe('PermissionWithGroup', () => {
  ...
  describe('check', () => {
    ...
    describe('when enabled', () => {
      it('updates isChecked when id matches', () => {
        expect(permissionWG.isEnabled).to.be.true;
        expect(permissionWG.isChecked).to.be.false;
        permissionWG.check(permissionWG.id);
        expect(permissionWG.isChecked).to.be.true;
      });
      it('updates isChecked when id matches', () => {
        expect(permissionWG.isEnabled).to.be.true;
        expect(permissionWG.isChecked).to.be.false;
      ...
      it('does not update isChecked when id does not match', () => {
        permissionWG
        expect(permissionWG.isEnabled).to.be.true;
        expect(permissionWG.isChecked).to.be.false;
        permissionWG.check(permissionWG.id);
      ...
      it('enables children when id matches', () => {
        permissionWG.children.push(permission1);
        permissionWG.children.push(permission2);
        expect(permissionWG.isEnabled).to.be.true;
        expect(permissionWG.isChecked).to.be.false;
        expect(permissionWG.children.every((child: Permission) => child.isEnabled)).to.be.false;
        permissionWG.check(permissionWG.id);
        expect(permissionWG.isChecked).to.be.true;
        expect(permissionWG.children.every((child: Permission) => child.isEnabled)).to.be.true;
      });
    });
  });
});
